...
> 章内位置：4 / 15「時間変化する磁束とFaraday則」
> このページでは、固定回路の変圧器起電力と、移動導体の運動起電力を区別します。
...
この現象を定式化したのがファラデーの法則です。次式で最初に扱う積分路は空間に固定されているとします。
...
だけでなく、動く導体内の電荷に働く磁気力による運動起電力です。回路要素の速度を
...
です。適切な条件の下で、動く回路が囲む磁束の全時間微分を使う磁束則へまとめられます。
...
で動くとき、棒の両端の起電力の大きさを求めよ。

6. 固定回路のFaraday誘導と運動起電力の源を、それぞれ一文で区別せよ。
...
$$
\mathcal E=\int_0^\ell
(\mathbf{u}\times\mathbf{B})\cdot d\mathbf{l}
$$
...
6. 固定回路では時間変化する磁場が非保存的電場を作ります。運動起電力では、磁場中を動く導体の電荷へ磁気Lorentz力が働きます。
...
- 次：[Ampère–Maxwell則](03_ampere_maxwell.md)
