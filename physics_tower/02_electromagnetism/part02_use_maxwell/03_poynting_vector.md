...
> 章内位置：9 / 15「エネルギー密度とPoyntingベクトル」
> 本文の単純なエネルギー密度式は真空、または線形・非分散・損失なし媒質に限定します。
...
と一般化できます。分散・損失媒質では、物質内部の自由度とのエネルギー交換を含むため、この瞬時式を無条件には使いません。
...
5. 真空平面波で、電場と磁場のエネルギー密度が等しいことを示せ。

6. 「Poyntingベクトルが非ゼロなら、その点で必ず不可逆なエネルギー損失が起きる」という主張を修正せよ。
...
$$
u_B=\frac{B^2}{2\mu_0}=\frac{E^2}{2\mu_0c^2}=\frac{\varepsilon_0E^2}{2}=u_E
$$

です。

6. Poyntingベクトルはエネルギー流束であり、損失そのものではありません。不可逆なJoule損失は
...
- 局所形と体積積分形の収支を同じ符号規約で説明できる。
- 真空式と物質中の一般表記を区別できる。
- エネルギー流と散逸を区別できる。

## 回路電力への接続

端子電力は、素子を囲む閉曲面を通るPoynting流束、場エネルギーの時間変化、物質への仕事へ分解できます。[Poyntingベクトルと回路電力](../part04_from_fields_to_circuits/08_power_and_poynting_flow.md)では、抵抗、充電中コンデンサ、コイルを同じ保存則で読みます。

## ナビゲーション

- 前：[電磁波](02_em_waves.md)
- 次：[誘電体](../part03_phenomenology/01_dielectrics.md)
